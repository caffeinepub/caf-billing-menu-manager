...
export default function SalesReports() {
  ...
  const isAnyFetching = dailyFetching || itemFetching || monthlyFetching || previousDayFetching;
  ...
  return (
    <div className="px-4 py-5 space-y-5">
      ...
      {/* Previous Day Sales History */}
      <section className="space-y-3">
        <h3 className="font-semibold text-sm text-foreground">Yesterday's Sales</h3>
        <PreviousDaySalesCard />
      </section>

      {/* Tabs for Item-wise and Monthly */}
      <Tabs defaultValue="items" className="space-y-4">
        <TabsList className="w-full h-10">
          <TabsTrigger value="items" className="flex-1 text-xs">Item-wise Sales</TabsTrigger>
          <TabsTrigger value="monthly" className="flex-1 text-xs">Monthly Sales</TabsTrigger>
        </TabsList>
